...
const Carousel: React.FC = () => {
  ...
  return (
    <>
      <ChevronLeftIcon
        className="h-13 w-13 absolute left-0 top-[20em] z-50 h-9 w-9 cursor-pointer select-none font-[100] text-white hover:text-stone-400 sm:left-10 md:left-12"
        onClick={prevSlide}
      />
      <ChevronRightIcon
        className="h-13 w-13 absolute right-0 top-[20em] z-50 h-9 w-9 cursor-pointer select-none rounded-xl text-white hover:text-stone-400 sm:right-10 md:right-12"
        onClick={nextSlide}
      />
      <div className="lightservicebg relative mx-auto flex min-h-[25em] max-w-[60%] cursor-pointer items-center rounded-[60px] px-1 sm:max-w-[50%] md:max-w-[40%]">
        {t.map((slide, index) => {
          return (
            <div
              className={
